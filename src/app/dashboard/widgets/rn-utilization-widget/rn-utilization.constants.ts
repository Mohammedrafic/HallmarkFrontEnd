
export const DefaultOptionFields = {
  text: 'name',
  value: 'id',
};

export const NOOFPERDIEMORDERS = 'No. of active Nursing as per the Employee Dept. Details screen.';
export const TARGETPERDIEMHOURS = '(Annual working hours divided by no. of days in current year) * Utilization * No. of active Nursing.';
export const ACTUALPERDIEMHOURS = 'Actual no. of hours scheduled for Nursing in the given date';
export const MASKPLACEHOLDER = "{ month: 'MM', day: 'DD', year: 'YYYY' }"
  ...
  SaveIrpOrder(
    ...
    return this.orderManagementService.saveIrpOrder(order).pipe(
      switchMap((order: Order[]) => {
        dispatch([
          new ShowToast(MessageTypes.Success,  order.length==1?'Order '+order[0].organizationPrefix?.toString()+'-'+order[0].publicId+' has been added':RECORD_ADDED),
      ...
      tap((payload) => {
        ...
        dispatch([
          order?.isQuickOrder
            ...
            : new ShowToast(MessageTypes.Success, 'Order '+ payload.organizationPrefix?.toString()+'-'+payload.publicId+' has been added'),

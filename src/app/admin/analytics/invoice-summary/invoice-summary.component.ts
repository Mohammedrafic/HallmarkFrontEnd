...
export class InvoiceSummaryComponent implements OnInit,OnDestroy {
  ...
  public onFilterControlValueChangedHandler(): void {
    this.bussinessControl = this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.BusinessIds) as AbstractControl;
    ...
    this.regionIdControl = this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.RegionIds) as AbstractControl;
    ...
    this.locationIdControl = this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.LocationIds) as AbstractControl;
    ...
    this.departmentIdControl = this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.DepartmentIds) as AbstractControl;
  ...
  public SearchReport(): void {
    let { startDate, endDate } = this.invoiceSummaryReportForm.getRawValue();
    this.paramsData =
    {
      "OrganizationParamISR": this.selectedOrganizations?.map((list) => list.name),
      "StartDateParamISR": formatDate(startDate, 'MM/dd/yyyy', 'en-US'),
      "EndDateParamISR": formatDate(endDate, 'MM/dd/yyyy', 'en-US'),
      "RegionParamISR": this.selectedRegions?.map((list) => list.name),
      "LocationParamISR": this.selectedLocations?.map((list) => list.name),
      "DepartmentParamISR": this.selectedDepartments?.map((list) => list.departmentName.trim())
    };
  ...
  public onFilterClearAll(): void {
    ...
    let startDate = new Date(Date.now());
    startDate.setDate(startDate.getDate() - 90);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.BusinessIds)?.setValue([]);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.RegionIds)?.setValue([]);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.LocationIds)?.setValue([]);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.DepartmentIds)?.setValue([]);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.StartDate)?.setValue(startDate);
    this.invoiceSummaryReportForm.get(analyticsConstants.formControlNames.EndDate)?.setValue(new Date(Date.now()));
    this.filteredItems = [];
  }
  public onFilterApply(): void {
    ...
    this.filteredItems = this.filterService.generateChips(this.invoiceSummaryReportForm, this.filterColumns);
    this.SearchReport();

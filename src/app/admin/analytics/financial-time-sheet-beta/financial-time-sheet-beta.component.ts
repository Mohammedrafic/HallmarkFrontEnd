...
@Component({
  selector: 'app-financial-time-sheet-beta',
  templateUrl: './financial-time-sheet-beta.component.html',
  styleUrls: ['./financial-time-sheet-beta.component.scss']
})
export class FinancialTimeSheetBetaComponent implements OnInit, OnDestroy {
  ...
  public reportName: LogiReportFileDetails = { name: "/JsonApiReports/FinancialTimeSheetBeta/FinancialTimeSheetBeta.wls" };
  public catelogName: LogiReportFileDetails = { name: "/JsonApiReports/FinancialTimeSheetBeta/FinancialTimeSheetBeta.cat" };
  ...
  public financialTimeSheetBetaFilterData$: Observable<CommonReportFilterOptions>;

  @Select(LogiReportState.commonReportCandidateSearch)
  public financialTimeSheetBetaCandidateSearchData$: Observable<SearchCandidate[]>;
  ...
  public financialTimesheetBetaReportForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.organizationId$.pipe(takeUntil(this.unsubscribe$)).subscribe((data: number) => {
      ...
      this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.AccrualReportTypes)?.setValue(1);
      this.onFilterControlValueChangedHandler();
      this.onFilterRegionChangedHandler();
      this.onFilterLocationChangedHandler();
      this.onFilterSkillCategoryChangedHandler();
      this.onFilterTimesheetStatusesChangedHandler();
      this.user?.businessUnitType == BusinessUnitType.Hallmark ? this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.BusinessIds)?.enable() : this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.BusinessIds)?.disable();
    });
  }
  ...
  private initForm(): void {
    ...
    this.financialTimesheetBetaReportForm = this.formBuilder.group(
      {
  ...
  public onFilterControlValueChangedHandler(): void {
    this.bussinessControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.BusinessIds) as AbstractControl;
    ...
    this.organizationData$.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      if (data != null && data.length > 0) {
        ...
        this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.BusinessIds)?.setValue(this.agencyOrganizationId);
        this.changeDetectorRef.detectChanges();
      }
    });

    this.bussinessControl.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.RegionIds)?.setValue([]);
      if (data != null && typeof data === 'number' && data != this.previousOrgId) {
        ...
        if (!this.isClearAll) {
          ...
          this.financialTimeSheetBetaFilterData$.pipe(takeWhile(() => this.isAlive)).subscribe((data: CommonReportFilterOptions | null) => {
            if (data != null) {
              this.isAlive = true;
              ...
              this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.TimesheetStatuses)?.setValue(this.defaultTimesheetStatuses);
  ...
  public onFilterRegionChangedHandler(): void {
    ...
    this.regionIdControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.RegionIds) as AbstractControl;
    this.regionIdControl.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      this.departments = [];
      this.locations = [];
      this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.LocationIds)?.setValue([]);
      this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.DepartmentIds)?.setValue([]);
  ...
  public onFilterLocationChangedHandler(): void {
    this.locationIdControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.LocationIds) as AbstractControl;
    ...
    this.departmentIdControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.DepartmentIds) as AbstractControl;
  ...
  public onFilterSkillCategoryChangedHandler(): void {
    this.skillCategoryIdControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillCategoryIds) as AbstractControl;
    this.skillCategoryIdControl.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      if (this.skillCategoryIdControl.value.length > 0) {
        ...
        this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillIds)?.setValue(skills.map((list) => list.id));
      }
      else {
        this.filterColumns.skillIds.dataSource = [];
        this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillIds)?.setValue([]);
      }
    });
    this.skillIdControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillIds) as AbstractControl;
  ...
  public onFilterTimesheetStatusesChangedHandler(): void {
    this.accrualReportTypesControl = this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.AccrualReportTypes) as AbstractControl;
    this.accrualReportTypesControl.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      if (data == 1) {
        this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.TimesheetStatuses)?.setValue(this.defaultTimesheetStatuses);
      }
      else {
        this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.TimesheetStatuses)?.setValue([]);
      }
    });
  }
  ...
  public SearchReport(): void {
    ...
    let { accrualReportTypes, businessIds, candidateName, candidateStatuses, departmentIds, jobId, jobStatuses, locationIds, orderTypes,
      regionIds, skillCategoryIds, skillIds, startDate, endDate, invoiceID, timesheetStatuses } = this.financialTimesheetBetaReportForm.getRawValue();
    if (!this.financialTimesheetBetaReportForm.dirty) {
  ...
  public onFilterDelete(event: FilteredItem): void {
    this.filterService.removeValue(event, this.financialTimesheetBetaReportForm, this.filterColumns);
  }
  ...
  public onFilterClearAll(): void {
    ...
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.LocationIds)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.DepartmentIds)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillCategoryIds)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.SkillIds)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.CandidateName)?.setValue(null);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.CandidateStatuses)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.OrderTypes)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.JobStatuses)?.setValue([]);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.StartDate)?.setValue(startDate);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.EndDate)?.setValue(new Date(Date.now()));
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.JobId)?.setValue(null);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.AccrualReportTypes)?.setValue(1);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.InvoiceID)?.setValue(null);
    this.financialTimesheetBetaReportForm.get(analyticsConstants.formControlNames.TimesheetStatuses)?.setValue(this.defaultTimesheetStatuses);
    this.filteredItems = [];
    this.locations = [];
    this.departments = [];
  ...
  public onFilterApply(): void {
    ...
    this.financialTimesheetBetaReportForm.markAllAsTouched();
    if (this.financialTimesheetBetaReportForm?.invalid) {
      return;
    }
    this.filteredItems = [];
    this.SearchReport();

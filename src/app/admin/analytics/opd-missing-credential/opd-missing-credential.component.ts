...
@Component({
  selector: 'app-opd-missing-credential',
  templateUrl: './opd-missing-credential.component.html',
  styleUrls: ['./opd-missing-credential.component.scss'],
})
export class OpdMissingCredentialComponent implements OnInit {
  ...
  public reportName: LogiReportFileDetails = { name: "/JsonApiReports/OPDMissingCredentials/OPDMissingCredentials.cls" };
  public catelogName: LogiReportFileDetails = { name: "/JsonApiReports/OPDMissingCredentials/OPDMissingCredentials.cat" };
  ...
  public opdMissingcredentialForm: FormGroup;
    ...
    this.organizationId$.pipe(takeUntil(this.unsubscribe$)).subscribe((data: number) => {
      ...
      if (this.user)
        if (this.user.businessUnitType == BusinessUnitType.Hallmark || this.user.businessUnitType == BusinessUnitType.MSP) {
          this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.BusinessIds)?.enable()
        } else { this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.BusinessIds)?.disable(); }
    });
  }
  ...
  selectPeriod(event: any) {
    let { startDate, period } = this.opdMissingcredentialForm.getRawValue();
    const value = event.itemData.id;
    this.periodIsDefault = this.opdMissingcredentialForm.controls['period'].value == "Custom" ? true : false;
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.startDate)?.setValue("");
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.endDate)?.setValue("");
    ...
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.startDate)?.setValue(startDateControl);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.endDate)?.setValue(new Date((endDateControl)));
  ...
  private loadperiod(): void {
    ...
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.Period)?.setValue("Custom");
  }
  public onFilterControlValueChangedHandler(): void {
    this.bussinessControl = this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.BusinessIds) as AbstractControl;
    ...
    this.agencyIdControl = this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.AgencyIds) as AbstractControl;
  ...
  public onFilterSkillCategoryChangedHandler(): void {
    ...
    this.skillCategoryIdControl = this.opdMissingcredentialForm.get(OPDMissingCredentialReportConstants.formControlNames.skillCategoryIds) as AbstractControl;
  ...
  public onFilterRegionChangedHandler(): void {
    this.regionIdControl = this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.RegionIds) as AbstractControl;
    this.regionIdControl.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe((data) => {
      this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.LocationIds)?.setValue([]);
      this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.DepartmentIds)?.setValue([]);
      this.locations = [];
      this.departments = [];
  ...
  public onFilterLocationChangedHandler(): void {
    this.locationIdControl = this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.LocationIds) as AbstractControl;
    ...
    this.departmentIdControl = this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.DepartmentIds) as AbstractControl;
  ...
  public onFilterDelete(event: FilteredItem): void {
    this.filterService.removeValue(event, this.opdMissingcredentialForm, this.filterColumns);
  }
  public onFilterClearAll(): void {
    ...
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.LocationIds)?.setValue([]);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.DepartmentIds)?.setValue([]);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.startDate)?.setValue(startDate);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.endDate)?.setValue(endDate);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.AgencyIds)?.setValue(this.defaultAgencys);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.CandidateStatuses)?.setValue(this.defaultCandidateStatuses);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.JobId)?.setValue('');
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.opcredFlag)?.setValue(false);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.Period)?.setValue("Custom");
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.SkillCategoryIds)?.setValue([]);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.SkillIds)?.setValue([]);
    this.opdMissingcredentialForm.get(analyticsConstants.formControlNames.CandidateName)?.setValue(null);
    this.filteredItems = [];
    this.locations = [];
    this.departments = [];
  ...
  public onFilterApply(): void {
    ...
    this.opdMissingcredentialForm.markAllAsTouched();
    if (this.opdMissingcredentialForm?.invalid) {
      return;
    }
    this.filteredItems = [];
    this.SearchReport();

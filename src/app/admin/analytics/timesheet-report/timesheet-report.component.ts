...
export class TimesheetReportComponent implements OnInit ,OnDestroy{
  ...
  public SearchReport(): void {
    let { startDate, endDate } = this.timesheetReportForm.getRawValue();
    this.paramsData =
    {
      "OrganizationParamTSR": this.selectedOrganizations?.map((list) => list.name),
      "StartDateParamTSR": formatDate(startDate, 'MM/dd/yyyy', 'en-US'),
      "EndDateParamTSR": formatDate(endDate, 'MM/dd/yyyy', 'en-US'),
      "RegionParamTSR": this.selectedRegions?.map((list) => list.name),
      "LocationParamTSR": this.selectedLocations?.map((list) => list.name),
      "DepartmentParamTSR": this.selectedDepartments?.map((list) => list.departmentName.trim())
    };
  ...
  public onFilterClearAll(): void {
    ...
    this.timesheetReportForm.get(analyticsConstants.formControlNames.EndDate)?.setValue(new Date(Date.now()));
    this.filteredItems = [];
  }
  public onFilterApply(): void {
    ...
    this.filteredItems = this.filterService.generateChips(this.timesheetReportForm, this.filterColumns);
    this.SearchReport();

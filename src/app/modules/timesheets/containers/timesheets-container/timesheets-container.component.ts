...
import { distinctUntilChanged, Observable, switchMap, takeUntil, filter, tap, of,
  combineLatest, debounceTime, take } from 'rxjs';
...
export class TimesheetsContainerComponent extends Destroyable implements OnInit {
  ...
  public handleChangeTab(tabIndex: number): void {
    ...
    if (this.filterService.canPreserveFilters()) {
      preservedFilters = this.isAgency ? this.store.selectSnapshot(PreservedFiltersState.preservedFiltersTimesheets)
    ...
    if (preservedFilters) {
      this.store.dispatch(new Timesheets.UpdateFiltersState(
        { statusIds: this.tabConfig[tabIndex].value, regionsIds: [...preservedFilters.regions],
  ...
  private initComponentState(): void {
    ...
    if (this.isAgency) {
      ...
      if (preservedFilters) {
        this.store.dispatch([
          new Timesheets.UpdateFiltersState({ regionsIds: [...preservedFilters.regions],
            locationIds: [...preservedFilters.locations]}),
          new Timesheets.GetFiltersDataSource(),
        ]);
      } else {
  ...
  private initOnRedirect(): void {
    const { timesheetId, organizationId } = this.location.getState() as { navigationId: number,
      timesheetId: number, organizationId: number };

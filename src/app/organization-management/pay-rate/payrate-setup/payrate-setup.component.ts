...
export class PayrateSetupComponent extends AbstractGridConfigurationComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public columnsToExport: ExportColumn[] = [
    ...
    { text: 'PayRateValue', column : 'PayRateValue' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.organizationStructure$
      ...
      .subscribe((structure: OrganizationStructure) => {
        ...
        this.regionSelected = [];
        this.regionSelected = this.allRegions.map(x => x.regionId);
      });
    ...
    this.payRateFilterFormGroup.get('regionIds')?.valueChanges.pipe(
    ...
    this.payRateFilterFormGroup.get('locationIds')?.valueChanges.pipe(
      takeUntil(this.unsubscribe$)
    ...
    this.payRateFilterFormGroup.get('departmentIds')?.valueChanges.pipe(
      takeUntil(this.unsubscribe$)
  ...
  public allRegionsChange(event: { checked: boolean }): void {
    ...
    this.regionSelected = [];
    this.PayRatesFormGroup.controls['locationIds'].setValue([]);

    this.regionSelected = this.orgRegions.map(x => x.regionId);
  }
  ...
  public allLocationsChange(event: { checked: boolean }): void {
    ...
    this.PayRatesFormGroup.controls['departmentIds'].setValue([]);
    this.cd.detectChanges();
    this.locationSelected = [];
    this.locationSelected = this.locations.map(x => x.locationId);
  }
  ...
  public allDepartmentsChange(event: { checked: boolean }): void {
    ...
    if (this.allDepartmentsSelected) {
      ...
      this.PayRatesFormGroup.controls['skillIds'].setValue([]);
    } else {
  ...
  private locationChangedHandler(): void {
    this.PayRatesFormGroup
      ...
      .subscribe((locationIds: number[]) => {
        this.locationSelected = [];
        this.locationSelected = locationIds;
        if (locationIds && locationIds.length > 0) {
          const departments: OrganizationDepartment[] = [];
          locationIds.forEach((id) => {
  ...
  private skillChangeHandler(): void {
    this.PayRatesFormGroup
      .get('skillIds')
      ?.valueChanges.pipe(takeUntil(this.unsubscribe$))

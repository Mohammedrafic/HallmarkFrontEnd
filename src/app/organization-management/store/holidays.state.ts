...
import { RECALCULATING_TRIGGERED, RECORD_ADDED, RECORD_DELETE, RECORD_MODIFIED } from "src/app/shared/constants/messages";
import { MessageTypes } from "src/app/shared/enums/message-types";
import { ShowToast } from "src/app/store/app.actions";
import { UserState } from 'src/app/store/user.state';
import { 
  CheckIfExist, DeleteHoliday, DeleteHolidaySucceeded, EditHoliday, ExportHolidays, GetAllMasterHolidays,
  GetHolidayDataSources, GetHolidaysByPage, SaveHoliday, SaveHolidaySucceeded,
...
export class HolidaysState {
  ...
  GetHolidaysByPage(
    { patchState }: StateContext<HolidaysStateModel>, { pageNumber, pageSize, orderBy, filter }: GetHolidaysByPage
  ): Observable<OrganizationHolidaysPage> {
  ...
  GetAllMasterHolidays({ patchState }: StateContext<HolidaysStateModel>): Observable<Holiday[]> {
    patchState({ isHolidayLoading: true });
  ...
  @Action(SaveHoliday)
  SaveHoliday(
    { patchState, dispatch }: StateContext<HolidaysStateModel>, { payload }: SaveHoliday
  ): Observable<OrganizationHoliday | void> {
    patchState({ isHolidayLoading: true });
    return this.holidaysService.saveUpdateOrganizationHoliday(payload).pipe(
      tap((payloadResponse) => {
        patchState({ isHolidayLoading: false });
        dispatch([
          new ShowToast(MessageTypes.Success, RECORD_ADDED + '\n' + RECALCULATING_TRIGGERED),
          new SaveHolidaySucceeded(payloadResponse),
        ]);
  ...
  DeleteHoliday(
    { patchState, dispatch }: StateContext<HolidaysStateModel>, { payload }: DeleteHoliday
  ): Observable<OrganizationHoliday | void> {
    patchState({ isHolidayLoading: true });
    return this.holidaysService.removeOrganizationHoliday(payload).pipe(tap((payload) => {
      ...
      dispatch([
        ...
        new DeleteHolidaySucceeded(payload),
      ]);
      return payload;
    }),
    catchError(() => dispatch(new ShowToast(MessageTypes.Error, 'Holiday cannot be deleted'))));
  }

  @Action(CheckIfExist)
  CheckIfExist({ patchState }: StateContext<HolidaysStateModel>, { payload }: CheckIfExist): Observable<boolean> {
  ...
  GetHolidayDataSources({ patchState }: StateContext<HolidaysStateModel>): Observable<string[]> {
